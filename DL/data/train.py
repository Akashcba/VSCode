...
Original file is located at
    https://colab.research.google.com/drive/1bP1ATlpCkYAq0NHW1Lx7BPxmkyZ_UfTp
...
torch.save(model,'train50k.pt')
from google.colab import files
files.download('train50k.pt')

"""## Predicting using dev set"""
...
"""Some predictions of train set """

prediction(dataset[25][0])

prediction(dataset[75][0])

prediction(dataset[222][0])

prediction(dataset[random.randint(0,50000)][0])
...
tdf.iloc[:,0] = tdf['Eng'].apply(func=cleanerEng)
tdf.iloc[:,1] = tdf['Hin'].apply(func= cleanerHindi)
tdf.iloc[:,0] = tdf['Eng'].apply(func= lambda x : (str(x).split()))
tdf.iloc[:,1] = tdf['Hin'].apply(func= lambda x : (str(x).split()))

tdf.iloc[:,0] = tdf['Eng'].apply(func= addTokens,start=False)
...
"""### Checking Predictions on dev set"""
